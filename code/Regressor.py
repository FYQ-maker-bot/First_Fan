...
def Phos1(img_dim1, img_dim2, init_form, dense_number, dropout_dense, weight_decay):
...
aaa2 = np.zeros(shape=(num_test, 100, 20))
bbb2 = np.zeros(shape=(num_test, 1547))
...
test_file_name = ""
win1 = 100
X_text, T3, raw3, length3, y_ture3 = getMatrixLabelh(test_file_name, win1)
X2_test = np.load("")
aaa2[:] = X_text[:]
bbb2[:] = X2_test[:]

aaa2 = aaa2.reshape((num_test, 2000))
...
nb_batch_size = 64  # 512
nb_classes = 2
nb_epoch = 200

model1 = Phos1(img_dim1, img_dim2, init_form, dense_number, dropout_dense, weight_decay)

opt = adam_v2.Adam(learning_rate=learning_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-08)

model1.compile(loss={'Dense_2': 'MSE'},
               optimizer=opt, metrics=['MSE'])

history = model1.fit([aaa, bbb], y=y_ture, batch_size=nb_batch_size,
